...
# Define your HoverNet architecture for binary classification
# Define HoverNet architecture

# Set parameters
IMAGE_SIZE = (256, 256)  # Resize images to 224x224
train_path = r'D:\TANG GEI KI\PHD Dataset Split\2. TRAIN DATASET SPLIT\TRAINING (80%)'  # Set the path to your training data
valid_path = r'D:\TANG GEI KI\PHD Dataset Split\2. TRAIN DATASET SPLIT\VALIDATION (10%)'  # Set the path to your validation data
...
# Load DenseNet121 pre-trained model without the top layer
...
model_checkpoint = ModelCheckpoint(r'E:\4. DENSENET121\4. H5 FILE\64 Adam Densenet121 (Epoch 200).h5', save_best_only=True, monitor='val_loss', mode='min', verbose=1)
...
history = model.fit(
    ...
    epochs=200,  # Set number of epochs as required
...
Y_test_pred_hot = (Y_test_pred > 0.5).astype("int32").flatten()

# Get true labels from test_generator
Y_test_actual_hot = test_generator.classes[:len(Y_test_pred_hot)]
...
print("\nClassification Report:")
print(classification_report(Y_test_actual_hot, Y_test_pred_hot, target_names=target_names))
